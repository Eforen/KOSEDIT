...
/**
 * @fileoverview JavaScript for Blockly kOS's Code application.
 ...
 * @private
 */
Code.TABS_ = ['blocks', 'kerboscript', 'xml'];
...
Code.tabClick = function(id) {
  ...
  // If the kOS pane was selected, validate blocks before switching.
  if (id == 'tab_kerboscript') {
    // Check for bad block configurations that make it unlikely that
    // the resulting code is correct.
    /*
    var badBlock = Blockly.kOS.getUnconnectedBlock();
    if (badBlock) {
      warningText = 'This block is not properly connected to other blocks.';
    } else {
      badBlock = Blockly.kOS.getBlockWithWarning();
      if (badBlock) {
    ...
    if (badBlock) { 
      ...
      // If they choose the other option ("generate kOS anyway"), the fake
      // tab "tab_kerboscript!" is selected, and this validation will get skipped.
      var style = {
        left: '25%',
        top: '5em'
      };
  ...
  // Hack: To skip the bad block checking, id = "tab_kerboscript!".
  if (id == 'tab_kerboscript!') {
    id = 'tab_kerboscript';
  }
  Code.displayTab(id);
...
Code.renderContent = function() {
  ...
  if (content.id == 'content_xml') {
    ...
    xmlTextarea.focus();
  } else if (content.id == 'content_kerboscript') {
    var code = Blockly.kOS.workspaceToCode();
    content.textContent = code;
    if (typeof prettyPrintOne == 'function') {
      code = content.innerHTML;
      code = prettyPrintOne(code, 'lang-kerboscript');
